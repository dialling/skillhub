...
const DICT: Dict = {
  ...
  'update.dataOnly': ['有更新的技能数据与目录', 'Newer skill data and catalog are available'],
  ...
  'detail.pickSkillsThenFolder': ['勾选要装的技能，点下面的按钮选一个目录，技能会直接从 GitHub 取最新版放进去', 'Tick the skills you want, then pick a folder — each one is fetched fresh from GitHub into its own subfolder'],
  ...
  'install.title': ['安装 {n} 个技能', 'Install {n} skills'],
  'install.titleOne': ['安装「{name}」', 'Install "{name}"'],
  'install.explain': [
  ...
  'install.suggested': ['设置里选的位置', 'Your choice in Settings'],
  'install.pickFirst': ['先选一个目录', 'Pick a folder first'],
  'install.confirm': ['装到这里', 'Install here'],
  ...
  'library.installPendingHint': ['有 {n} 个技能还没安装，点一下选个目录就能装', '{n} skills are not installed yet — pick a folder and they go there'],
  'library.installedTotal': ['已安装 {n} 个技能', '{n} skills installed'],
  'library.open': ['打开', 'Open'],
  'library.heroNotInstalled': ['尚未安装 —— 点「安装仓库」选个目录就会装好', 'Nothing installed yet — Install repo picks a folder and puts it there'],
  ...
  'profile.src.settings': ['个人访问令牌（设置里填写）', 'Personal access token (set in Settings)'],
  'profile.src.env': ['环境变量 GH_TOKEN / GITHUB_TOKEN', 'Environment variable GH_TOKEN / GITHUB_TOKEN'],
  'profile.src.none': ['未登录', 'Not signed in'],
  ...
  'settings.installLocation': ['推荐的技能安装位置', 'Suggested skills location'],
  'settings.installLocationHint': [
    '这是推荐位置：安装时技能仍会写进每个已启用智能体自己的技能目录，因为智能体只读自己的目录。留空则自动检测。',
    'This is a recommendation: an install still goes into each enabled agent\'s own skills directory, because that is the only place it reads. Leave unset to auto-detect.'
  ],
  'settings.changeLocation': ['更改位置', 'Change location'],
  'target.title': ['推荐的技能安装位置', 'Suggested skills location'],
  'target.intro': ['这是给技能挑位置时的建议值，不是安装目的地 —— 每次安装都由你在弹窗里选。已根据你电脑上的情况给出，可以随时更改。', 'This is advice for the destination picker, not a destination itself — you choose the folder on every install. Based on what is already on your machine; change it any time.'],
  'target.reasonConfigured': ['你之前选定的推荐位置。', 'The location you picked earlier as the recommendation.'],
  ...
  'target.saved': ['已记住推荐位置：{path}', 'Remembered as the suggested location: {path}'],
  ...
  'toast.uninstalledShared': [
    ...
    'Uninstalled — these agents share one directory and lost it too: {agents}'
  ],
  ...
  'toast.catalogUpdated': ['技能目录已更新到 {version}', 'Skill catalog updated to {version}'],
