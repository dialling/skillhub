...
const DICT: Dict = {
  ...
  'card.appNeedsHint': ['这些技能属于应用 {app}，需要用它的运行环境（守护进程、凭据、依赖）', 'These skills belong to the application {app} and need its runtime (daemon, credentials, dependencies)'],
  ...
  'launch.notInstalledClient': ['这个技能还没装到 {agent}。它是客户端，无法自动打开工作区，所以工作区里的副本它可能看不到。', 'This skill is not installed in {agent}. As a client it cannot be opened at the workspace, so it may never see the copy placed there.'],
  'launch.willInstallGlobally': ['启动时会同时把技能装到 {agent} 自己的技能目录 —— 它是客户端，无法自动打开工作区，只放在工作区里它可能看不到。', 'Launching also installs this skill into {agent}\'s own skills directory: as a client it cannot be opened at the workspace, so a copy placed only there may never be seen.'],
  'launch.needsGlobalInstall': ['{agent} 无法自动打开工作区 —— 技能只装在工作区里，它可能找不到。装到它自己的技能目录就一定可见。', '{agent} cannot be opened at the workspace automatically, so the skill may only exist in a folder it never opens. Installing it into the agent\'s own directory makes it findable.'],
  'launch.installNow': ['立即安装', 'Install now'],
